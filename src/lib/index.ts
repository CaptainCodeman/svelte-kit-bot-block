...
export type Http4xx = 400 | 401 | 402 | 403 | 404
...
export interface Options {
	...
	// http status to send to blocked requests (default 404)
	blocked_status: Http4xx
}
...
export const createHandler = (options?: Partial<Options>): Handle => {
	...
	const handle: Handle = async ({ event, resolve }) => {
		...
		if (block(isIP(hostname) > 0, 'ip address', hostname)) {
			return new Response(null, { status: opts.blocked_status })
		}
